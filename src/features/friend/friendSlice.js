...
const initialState = {
  ...
  totalPages: 1,
  OutgoingPageUsers: [],
  incomingTotalUsers: 0,
  outgoingTotalUsers: 0,
};
...
const slice = createSlice({
  ...
  reducers: {
    ...
    getFriendsRequestSuccess(state, action) {
      ...
      users.forEach((user) => (state.usersById[user._id] = user));
      state.currentPageUsers = users.map((user) => user._id);
      state.incomingTotalUsers = count;
      state.totalUsers = state.incomingTotalUsers + state.outgoingTotalUsers;
      state.totalPages = totalPages;
    },
    getFriendsOutgoingRequestSuccess(state, action) {
      state.isLoading = false;
      state.error = null;

      const { users, count, totalPages } = action.payload;
      users.forEach((user) => (state.usersById[user._id] = user));
      state.OutgoingPageUsers = users.map((user) => user._id);
      state.outgoingTotalUsers = count;
      state.totalUsers = state.incomingTotalUsers + state.outgoingTotalUsers;
      state.totalPages = totalPages;
    },
...
export const getFriendsOutgoingRequest =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      dispatch(slice.actions.getFriendsOutgoingRequestSuccess(Response.data));
    } catch (error) {
      dispatch(slice.actions.hasError(error.message));
      toast.error(error.message);
    }
